...
export const dynamic = "force-dynamic"; // ensure fresh grouping on each request

export default async function PropertiesPage() {
  const rows: MojoListing[] = await fetchMojoListings();
  const props: Property[] = rows.map(mapMojoToProperty);

  // Group by normalized building label
  const byBuilding = rows.reduce<Record<string, { label: string; items: Property[] }>>(
    (acc, row, idx) => {
    ...
    <section className="container py-12">
      ...
      {groups.length === 0 && (
        <p className="mt-4 text-sm text-gray-600">
          No listings returned from ShowMojo right now.
        </p>
      )}
